...
import {getFullSignedMeetingList, getSignedMeetingListByMemberEmail} from "../../services/client.js";
import {LoginAuth} from "../context/LoginContext.jsx";

const SignedMeetingList = () => {

    const {memberInfo} = LoginAuth();
    const [searchText, setSearchText] = useState('');
    const [searchedColumn, setSearchedColumn] = useState('');
    const [signedMeetingList, setSignedMeetingList] = useState([]);

    const fetchSignedMeetingList = async () => {
        try {
            if (memberInfo.memberRoles == "ACADEMIC") {
                const res = await getFullSignedMeetingList();
                setSignedMeetingList(res.data);
            } else {
                const res = await getSignedMeetingListByMemberEmail(memberInfo.email);
                setSignedMeetingList(res.data);
            }
        } catch (error) {
    ...
    const columns = signedMeetingList.length > 0 ? Object.keys(signedMeetingList[0]).slice(1).map((key) => ({
        title: formatKey(key),
        dataIndex: key,
    ...
    const modifiedData = signedMeetingList.map(item => {
    ...
    return (
        <>
            <HomeNav/>
            <Divider>Signed Meeting List</Divider>
            <Table
                columns={columns}
